    ...
    if HRSC_Object != None:
        for HRSC_Object in HRSC_Objects.iter('HRSC_Object'):
            ...
            if Class_ID in ship:
                ...
                b = (float(HRSC_Object.find('mbox_cx').text), float(HRSC_Object.find('mbox_cy').text),
                     float(HRSC_Object.find('mbox_w').text), float(HRSC_Object.find('mbox_h').text),float(HRSC_Object.find('mbox_h').text))
                b1, b2, b3, b4,b5 = b
                # 标注越界修正
                if b2 > w:
                    b2 = w
