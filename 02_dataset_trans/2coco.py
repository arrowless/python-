...
parser.add_argument('--root_dir', default='/Users/arrow/数据集/HRSC2016_dataset/HRSC2016/FullDataSet',type=str, help="root path of images and labels, include ./images and ./labels and classes.txt")
...
def yolo2coco(arg):
    ...
    if arg.random_split or arg.split_by_file:
        ...
        for i, cls in enumerate(classes, 0):
            train_dataset['categories'].append({'id': i, 'name': cls, 'supercategory': 'none'})
            val_dataset['categories'].append({'id': i, 'name': cls, 'supercategory': 'none'})
    ...
    for k, index in enumerate(tqdm(indexes)):
        # 支持 png jpg 格式的图片。
        txtFile = index.replace('images','txt').replace('.jpg','.txt').replace('.bmp','.txt')
        # 读取图像的宽和高
