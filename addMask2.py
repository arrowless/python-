...
"""
anno_dir = '/Users/arrow/数据集/RSOD-split/aircraft/Annotation/xml/'
image_dir = '/Users/arrow/数据集/RSOD-split/aircraft/JPEGImages/'
mask_image_dir = '/Users/arrow/数据集/RSOD-split/aircraft/maskImage/'
mask_xml_dir = '/Users/arrow/数据集/RSOD-split/aircraft/maskxml/'
occlusion_dir = '/Users/arrow/数据集/RSOD-split/aircraft/occlusionImage/'
Ratio = [1 / 4, 1 / 2, 3 / 4]
num_mask_ratio = [1 / 4, 1 / 2, 3 / 4]
IsImage = False
IsFilling = True
...
def _main():
    ...
    for file in filelist:
        ...
        if IsFilling:
            _Mask_Filling(annos, anno_file, Ratio[occ])
        print(file + ': ' + str(num))
...
def _Mask_Filling(annotation, file, ratio):
    """
    ...
    """
    """使用某一颜色的色块对目标遮挡"""
    while None in annotation:
        annotation.remove(None)
    print(annotation[0]['xmin'], annotation[0]['ymin'], annotation[0]['xmax'], annotation[0]['ymax'])
    filenum = os.path.splitext(file)
    filename = filenum[0] + '.jpg'
    w = [[]] * len(annotation)
    h = [[]] * len(annotation)
    ...
    for i in range(len(annotation)):
        ...
        box[i] = (annotation[i]['xmin'], annotation[i]['ymin'], annotation[i]['xmax'], annotation[i]['ymax'])
        print(box)
        w[i] = box[i][2] - box[i][0]
        print("w:", w[i])
        h[i] = box[i][3] - box[i][1]
        ...
        # 获取色块，(0, 0, 0)即黑色
        mask[i] = Image.new('RGBA', (w[i], h[i]), (0, 0, 0))
        # 对目标分别进行左部分、右部分、上部分、下部分位置进行遮挡
        if seed == 0:
            place[i] = (box[i][0], box[i][1], int(box[i][2] - (1 - ratio) * w[i]), box[i][3])
            # print('place',place[i])
            size[i] = [place[i][2] - place[i][0], place[i][3] - place[i][1]]
            pil_im_crop_resize = mask[i].resize(size[i])
            pil_im.paste(pil_im_crop_resize, place[i])
        elif seed == 1:
            place[i] = (int(box[i][0] + ratio * w[i]), box[i][1], box[i][2], box[i][3])
            size[i] = (place[i][2] - place[i][0], place[i][3] - place[i][1])
            pil_im_crop_resize = mask[i].resize(size[i])
            pil_im.paste(pil_im_crop_resize, place[i])
            ...
            # pil_im = Image.open(image_dir + filename)
            place[i] = (box[i][0], box[i][1], box[i][2], int(box[i][3] - (1 - ratio) * h[i]))
            size[i] = (place[i][2] - place[i][0], place[i][3] - place[i][1])
            pil_im_crop_resize = mask[i].resize(size[i])
            pil_im.paste(pil_im_crop_resize, place[i])
        elif seed == 3:
            pil_im = Image.open(image_dir + filename)
            place[i] = (box[i][0], int(box[i][1] + ratio * h[i]), box[i][2], box[i][3])
            size[i] = (place[i][2] - place[i][0], place[i][3] - place[i][1])
            pil_im_crop_resize = mask[i].resize(size[i])
            pil_im.paste(pil_im_crop_resize, place[i])
        else:
            pass
    pil_im.save(mask_image_dir + filenum[0] + '_' + '10' + '.jpg')
    copyfile(anno_dir + file, mask_xml_dir + filenum[0] + '_' + '10' + '.xml')
    pil_im.close()
